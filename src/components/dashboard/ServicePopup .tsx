...
const ServicePopup = ({ service, onClose, onOpenConsultations }: ServicePopupProps) => {
  ...
  const NextArrow = ({ onClick }: any) => (
    <button
      onClick={onClick}
      className="absolute right-2 top-1/2 -translate-y-1/2 z-20 w-10 h-10 bg-white/90 backdrop-blur-sm rounded-full shadow-lg flex items-center justify-center hover:bg-primary hover:text-white transition-all duration-300 border border-gray-100 hover:border-primary group"
          ...
          html: `
            <div style="text-align: ${isRTL ? 'right' : 'left'}; direction: ${isRTL ? 'rtl' : 'ltr'};" class="font-ElMessiri">
              ...
              <div class="bg-green-50 ${isRTL ? 'border-r-4' : 'border-l-4'} border-green-400 p-4 rounded-lg">
                ...
                <p class="text-green-700 text-sm mt-2">${t('dashboard.wallet.pointsUsed')} ${Math.ceil(totalPrice * pointsPerRiyal).toLocaleString()} ${t('dashboard.wallet.point')}</p>
              </div>
            </div>
          `,
  ...
  return (
    ...
    >
      <div className={`relative rounded-3xl w-[90%] w-5xl h-[90vh] p-3 overflow-y-auto shadow-2xl ${
        isDarkMode ? 'bg-slate-800' : 'bg-white'
      }`}>
        {/* Close Button */}
        <button
          onClick={onClose}
          className={`absolute top-63 ${isRTL ? 'left-3' : 'right-3'} hover:text-red-500 text-2xl w-10 h-10 flex items-center justify-center rounded-full transition-all duration-200 z-30 ${
            isDarkMode 
